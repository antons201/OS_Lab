...
void crc(char *fileName, int expectedRes) {
    const unsigned short crc16Table[256] = { //таблица значения для вычисления crc16
    ...
    while (fread(&buff, 1, 1, file) != 0) { //пока не считали весь файл читаем по байту и считаем чексумму
        crc = (crc >> 8) ^ crc16Table[(crc & 0xFF) ^ buff]; //считаем чексумму
    }
    ...
    printf("Result checksum %d, expected result %d\n", crc, expectedRes); //выводим полученную и ожидаемую чексумму
    printf("Time %ld мс\n", get_time_difference(tv1.tv_sec, tv1.tv_usec, tv2.tv_sec, tv2.tv_usec)); // выводим время
}

int main() {
    crc("../tests/test1.txt", 22775); //Тест 1
    crc("../tests/test2.txt", 39561); //Тест 2
    crc("../tests/test3.txt", 24406); //Тест 3
    crc("../tests/test4.txt", 52973); //Тест 4
    return 0;
}
